...
router.post("/mint", async (req: Request<NFTMetadata>, res) => {
  ...
  if (!metadata) return res.status(400).send("Metadata is required");
  if (!metadata.name) return res.status(400).send("Name is required");
  if (!metadata.description)
    return res.status(400).send("Description is required");
  if (!metadata.image) return res.status(400).send("Image is required");
  ...
  if (!metadata.power) return res.status(400).send("Power is required");
  if (!metadata.health) return res.status(400).send("Health is required");
  if (!metadata.agility) return res.status(400).send("Agility is required");
  ...
  const results = await contract.erc1155.lazyMint(metadatas as any); // uploads and creates the NFTs on chain
  res.send(results);
});
...
router.post("/transfer", async (req, res) => {
  ...
  if (!address) return res.status(400).send("To address is required");
...
router.post("/setClaimingConditions", async (req, res) => {
  ...
  const sdk = getPrivateSdk();
  const contract = await getContractAsync(sdk);
  const result = await contract.erc1155.claimConditions.set(
    tokenId,
    claimConditions
  );
  res.send(result);
